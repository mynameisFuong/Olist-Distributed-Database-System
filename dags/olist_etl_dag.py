...
def upload_to_hdfs():
    ...
    # Tạo thư mục HDFS
    requests.put(f"{WEBHDFS_URL}{HDFS_RAW_DIR}?op=MKDIRS&user.name=root")

    # Upload file qua WebHDFS
    r = requests.put(
        f"{WEBHDFS_URL}{HDFS_RAW_DIR}/{filename}"
        "?op=CREATE&user.name=root&overwrite=true",
    ...
    print(f"Upload OK -> {HDFS_RAW_DIR}/{filename}")
